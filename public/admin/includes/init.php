...
define('ECTOUCH_PATH', str_replace('\\', '/', dirname(dirname(dirname(__DIR__)))) . '/');

$db_config = require(ECTOUCH_PATH . 'config/database.php');
$db_config = parseDsn($db_config);
$db_host   = $db_config['DB_HOST'] . ":" . $db_config['DB_PORT'];
$db_name   = $db_config['DB_NAME'];
$db_user   = $db_config['DB_USER'];
$db_pass   = $db_config['DB_PWD'];
...
if (!file_exists(ECTOUCH_PATH . 'storage/temp/caches'))
{
    @mkdir(ECTOUCH_PATH . 'storage/temp/caches', 0777);
    @chmod(ECTOUCH_PATH . 'storage/temp/caches', 0777);
}

if (!file_exists(ECTOUCH_PATH . 'storage/temp/compiled/admin'))
{
    @mkdir(ECTOUCH_PATH . 'storage/temp/compiled/admin', 0777);
    @chmod(ECTOUCH_PATH . 'storage/temp/compiled/admin', 0777);
}
...
$smarty->compile_dir   = ECTOUCH_PATH . 'storage/temp/compiled/admin';
if ((DEBUG_MODE & 2) == 2)
{
...
function parseDsn($dsnStr) {
  ...
  $info = parse_url($dsnStr['dsn']);
  if(!$info) {
      return false;
  }

  parse_str(str_replace(';', '&', $info['path']), $info['path']);

  return array(
      'DB_TYPE' => $info['scheme'],
      'DB_HOST' => isset($info['path']['host']) ? $info['path']['host'] : '',
      'DB_USER' => isset($dsnStr['username']) ? $dsnStr['username'] : '',
      'DB_PWD' => isset($dsnStr['password']) ? $dsnStr['password'] : '',
      'DB_NAME' => isset($info['path']['dbname']) ? $info['path']['dbname'] : '',
      'DB_PREFIX' => isset($dsnStr['tablePrefix']) ? $dsnStr['tablePrefix'] : 'ecs_',
      'DB_PORT' => isset($dsnStr['port']) ? $dsnStr['port'] : '3306',
      'DB_CHARSET' => isset($dsnStr['charset']) ? $dsnStr['charset'] : 'utf8',
  );
}

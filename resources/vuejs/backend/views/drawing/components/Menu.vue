  ...
  <div class='menu'>
    <header>
      ...
      <nav class="header-nav">
        <ul>
          <li class="header-btn" v-for="(item, index) in menuComponent" v-show="item.isShow" @mouseenter="navHoverTab(index,$event)"
            @mouseleave.stop="navOutTab(index)">
            ...
            <div class="sub-menu" :class="{active:item.isActive}">
              <ul>
                <draggable :options="options" @clone="onDraggClone($event)">
                  <li v-for="itemSub in item.subComponent" v-show="itemSub.isShow" :data-module="itemSub.module">
